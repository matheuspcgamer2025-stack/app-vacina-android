...
function renderizarListaFiltrada(textoPesquisa) {
    ...
    filtradas.forEach((vax, index) => {
        const jaTomou = nomesJaTomados.some(nome => nome.includes(vax.nome.toLowerCase()));
        
        const card = document.createElement('div');
        card.className = `card accordion-card vax-item ${jaTomou ? 'applied' : 'pending'}`;
        ...
        card.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; width:100%;">
                <div class="vax-info" style="flex:1;">
                    ...
                    <p style="margin:2px 0 0 0; font-size:13px; color:var(--text-secondary);">${vax.dose} | ${vax.recomendacao}</p>
                </div>
                <div style="display:flex; align-items:center; gap:10px;">
                    <span class="status-tag" style="font-size:12px; font-weight:bold; color: ${jaTomou ? 'var(--success)' : '#f0ad4e'}">
                        ${jaTomou ? 'Tomada ✔️' : 'Pendente ⏳'}
                    </span>
