...
const EMOJIS_DIALOGO = [
    '❌', '⚠️', '✅', '🎉', '📩', '📌', '🚨', '⏳', '📅', '📍', '🕒', '📡',
    '👥', '💉', '🏠', '💳', '🔔', '👤', '🗺️', '📄', '🔍', '📈', '🏃', '🥗',
    '🚗', '🏢', '📞', '👶', '✏️', '🚀', '🛡️', '🔒', '🎯'
];

function removerEmojis(mensagem) {
    let texto = String(mensagem || '');
    EMOJIS_DIALOGO.forEach((emoji) => {
        texto = texto.split(emoji).join(' ');
    });
    return texto;
}
...
function normalizarMensagemPorTipo(tipo, mensagem) {
    let texto = normalizarEspacos(limparPrefixoCategoria(removerEmojis(mensagem)));
...
export function setupAppDialogs() {
    ...
    window.alert = function(mensagem) {
        const mensagemSanitizada = removerEmojis(mensagem);

        if (!garantirElementos()) {
            alertaNativo(mensagemSanitizada);
            return;
        }

        alertQueue.push(String(mensagemSanitizada || ''));
        processarFilaAlertas();
    };

    window.confirm = function(mensagem) {
        // Evita quebrar legados ainda nao migrados para appConfirm
        return confirmNativo(removerEmojis(mensagem));
    };
}
