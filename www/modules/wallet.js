    ...
    novoForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (proximaDoseCalculada) {
                const dataBr = formatarDataBrDayjs(proximaDoseCalculada) || '--/--/----';
...
export async function carregarDadosDoFirebase(onUpdateCallback) {
    ...
    try {
        ...
        if (!usuarioAtual) {
            // Sessao local offline: nao tenta ler nuvem sem auth Firebase valido.
            return;
        }

        const uidAtual = usuarioAtual.uid || null;
        const emailAtual = String(usuarioAtual.email || '').trim().toLowerCase();
        ...
        if (emailAtual) {
            consultas.push(
                getDocs(query(collection(db, "vacinas"), where("usuarioId", "==", emailAtual)))
            );
        }
...
export function renderizarCarteira(onDeleteCallback) {
    ...
    vacinasFiltradas.forEach(vax => {
        const dataFormatada = formatarDataBrDayjs(vax.data) || '--/--/----';
        const card = document.createElement('div');
        card.className = "card vax-item applied";
        
        let textoProximaDose = "";
        if (vax.proximaDose) {
            const proxBr = formatarDataBrDayjs(vax.proximaDose) || '--/--/----';
    ...
    const linhas = [
        ...
        ...vacinasFiltradas.map((vax) => {
            ...
            const reforco = vax.proximaDose
                ? ` | Reforco: ${formatarDataBrDayjs(vax.proximaDose) || '--/--/----'}`
                : '';
