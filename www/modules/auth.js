...
function observarSessaoAutenticada(screenLogin, appMain, onLoginSuccess) {
    if (!screenLogin || !appMain) return;
    if (screenLogin.dataset.authObserverBound === '1') return;

    screenLogin.dataset.authObserverBound = '1';

    onAuthStateChanged(auth, (usuario) => {
        if (!usuario) return;

        const emailUsuario = String(usuario.email || '').trim().toLowerCase();
        const principal = emailUsuario || String(appState.usuarioLogado || '').trim() || 'usuario_autenticado';
        const identificadorOrigem = emailUsuario || principal;

        if (
            screenLogin.classList.contains('hidden') &&
        ...
        concluirLogin(identificadorOrigem, emailUsuario || null, screenLogin, appMain, onLoginSuccess);
    });
}

export async function encerrarSessaoUsuarioAtual() {
    try {
        await signOut(auth);
    } catch (_) {
        // Mesmo sem logout remoto, limpamos estado local para retorno seguro à tela de login.
    }

    appState.usuarioLogado = null;
    appState.identificadoresUsuario = [];
